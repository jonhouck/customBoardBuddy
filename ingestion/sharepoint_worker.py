    ...
    for attempt in range(4):
        try:
            ...
            if response.status_code == 200:
                # If downloading a file, return bytes, else return json
                if "application/json" in response.headers.get("Content-Type", ""):
                    return response.json()
                return response.content
            elif response.status_code == 429:
                # Rate limit handling (use Retry-After header if available, default 10s base)
...
def fetch_sharepoint_files(token: str, drive_id: str, limit: int = 10) -> list[dict]:
    """Fetch file metadata from the specified SharePoint Document Library (Drive)."""
    # Using the /children endpoint to get items in root. Can modify for specific folders.
    url = f"https://graph.microsoft.com/v1.0/drives/{drive_id}/root/children?$top={limit}"
    
    files = []
    
    while url and len(files) < limit:
        print("Fetching SharePoint page...")
        ...
        for item in data.get("value", []):
            ...
            if "file" in item:
                # Only care about PDFs for now
                if item.get("name", "").endswith(".pdf"):
                    files.append(item)
                    if len(files) >= limit:
                        break
...
def process_sharepoint(limit: int = 2):
    """Main workflow to process historical SharePoint documents."""
    ...
    print(f"Fetching up to {limit} PDF files from SharePoint drive {SHAREPOINT_DRIVE_ID}...")
    files = fetch_sharepoint_files(token, SHAREPOINT_DRIVE_ID, limit=limit)
    ...
    for f in files:
        ...
        for chunk in chunks:
            # We default matter_status to "Unknown" since SharePoint doesn't natively have this
            # in a flat file unless we extract it with AI.
            # Same for document_type, defaulting to "Historical Attachment"
            doc = {
                "id": str(uuid.uuid4()),
                "chunk_text": chunk,
...
if __name__ == "__main__":
    import argparse
    parser_arg = argparse.ArgumentParser(description="Ingest historical documents from SharePoint")
    parser_arg.add_argument("--limit", type=int, default=2, help="Number of files to fetch for testing")
    args = parser_arg.parse_args()
    
    process_sharepoint(limit=args.limit)
